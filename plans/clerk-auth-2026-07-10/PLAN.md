...
| Where | Name | Notes |
|---|---|---|
| client (Vercel + `.env.local`) | `VITE_CLERK_PUBLISHABLE_KEY` | publishable, safe to expose; the `VITE_` prefix is required — Vite only embeds `VITE_*` vars into the bundle |
...
1. Vercel: add `VITE_CLERK_PUBLISHABLE_KEY` env var (must keep the `VITE_` prefix), redeploy client.
